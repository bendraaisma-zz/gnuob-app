...
public class ContentViewOrEditPanel extends Panel {
   ...
   class ContentListDataProvider extends ListDataProvider<Content> {
      ...
      protected List<Content> getData() {
         return ((Category) ContentViewOrEditPanel.this.getDefaultModelObject()).getContents();
      }
   }
   ...
   private ImageUploadPanel imageUploadPanel = new ImageUploadPanel("imageUploadPanel", Model.of(new Content())) {
      ...
      public void uploadedImage(AjaxRequestTarget target) {
         ((Category) ContentViewOrEditPanel.this.getDefaultModelObject()).getContents().add((Content) this.getDefaultModel().getObject());
         target.add(getPage());
      }
   };

   public ContentViewOrEditPanel(final String id, final IModel<Category> model) {
      super(id, model);
   }
}
