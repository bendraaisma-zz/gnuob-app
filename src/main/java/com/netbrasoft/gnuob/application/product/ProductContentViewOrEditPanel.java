...
public class ProductContentViewOrEditPanel extends Panel {
   ...
   class SaveAjaxButton extends BootstrapAjaxButton {
      ...
      public SaveAjaxButton(Form<?> form) {
         super("save", Model.of(ProductContentViewOrEditPanel.this.getString("saveMessage")), form, Buttons.Type.Primary);
         setSize(Buttons.Size.Small);
         add(new LoadingBehavior(Model.of(ProductContentViewOrEditPanel.this.getString("saveMessage"))), new TinyMceAjaxSubmitModifier());
      }
      ...
      protected void onError(AjaxRequestTarget target, Form<?> form) {
         ...
         target.add(form);
         target.add(SaveAjaxButton.this.add(new LoadingBehavior(Model.of(ProductContentViewOrEditPanel.this.getString("saveMessage")))));
      }
      ...
      protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
         try {
            ...
            if (content.getId() == 0) {
               ((Product) markupContainer.getDefaultModelObject()).getContents().add(content);
            }
   ...
   private static final Logger LOGGER = LoggerFactory.getLogger(ProductContentViewOrEditPanel.class);

   private final MarkupContainer markupContainer;

   public ProductContentViewOrEditPanel(final String id, final IModel<Content> model, final MarkupContainer markupContainer) {
      super(id, model);
